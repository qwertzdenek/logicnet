...
public class PostDaoJpa extends GenericDaoJpa<Post, Long> implements PostDao {
    ...
    public List<Post> getMostPopularDay() {
        //TypedQuery<Post> q = em.createQuery("SELECT p FROM Post p WHERE p.dateTime > :dayBefore order by p.likeCount ASC, p.dateTime DESC", Post.class);
        TypedQuery<Post> q = em.createQuery("SELECT p FROM Post p order by p.likeCount ASC, p.dateTime DESC", Post.class);
